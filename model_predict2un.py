...
# Define class names (make sure they are in the same order as training)
class_names = ["skincancer", "unknown"]
...
# Load same pretrained EfficientNet used during training
efficientnet_model = EfficientNetB0(
    input_shape=(100, 100, 3),
    include_top=False,
    weights='efficientnet-b0_weights_tf_dim_ordering_tf_kernels_autoaugment_notop.h5'
)
...
model = load_model("efficientnet_skin_cancer_unknown2.weights.h5", compile=False)
#model.load_weights("efficientnet_skin_cancer_unknown2.weights.h5")
...
def pred_skin_disease3(img_path):
# Path to the image
            image_path =img_path
